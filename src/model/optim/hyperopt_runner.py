...
SEARCH_SPACES = {
    ...
    "rf": {
        ...
        "min_samples_leaf": scope.int(hp.quniform("rf_min_samples_leaf", 2, 10, q=2)),
        "min_samples_split": scope.int(hp.quniform("rf_min_samples_split", 3, 15, q=2)),
        "max_features": hp.quniform("max_features", 0.5, 0.9, q=0.1),
    },
    "xgb": {
        "max_depth": scope.int(hp.quniform("xgb_max_depth", 3, 12, q=3)),
        "min_child_weight": scope.int(hp.quniform("xgb_min_child_weight", 0, 10, q=2)),
        "learning_rate": hp.loguniform("xgb_learning_rate", -3, -0.5),
        "gamma": hp.quniform("xgb_gamma", 0, 10, q=2),
        "subsample": hp.quniform("xgb_subsample", 0.5, 0.8, q=0.1),
        "colsample_bytree": hp.quniform("xgb_colsample_bytree", 0.5, 0.8, q=0.1),
    },
    "mlp": {
    ...
    "cnn": {
        ...
        "base_channels": scope.int(hp.quniform("cnn_base_channels", 16, 64, 16)),
        "channel_growth": hp.quniform("cnn_channel_growth", 1.5, 2.1, 0.1),
    ...
    "resnet": {
        ...
        "base_channels": scope.int(hp.quniform("resnet_base_channels", 16, 64, 16)),
        "lr": hp.loguniform("resnet_lr", -7.5, -3),
    },
    "convnext": {
        ...
        "dim_growth": hp.uniform("convnext_dim_growth", 1.1, 1.5),
        "lr": hp.loguniform("convnext_lr", -7.5, -3),
    },
}
