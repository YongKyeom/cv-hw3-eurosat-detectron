...
SEARCH_SPACES = {
    ...
    "mlp": {
        "num_layers": scope.int(hp.quniform("mlp_layers", 3, 6, 1)),
        "first_dim": scope.int(hp.quniform("mlp_first_dim", 128, 2048, 64)),
        "width_decay": hp.uniform("mlp_width_decay", 0.3, 0.9),
        "dropout": hp.quniform("mlp_dropout", 0.1, 0.5, q=0.1),
        "lr": hp.loguniform("mlp_lr", -9, -5),
        "epochs": scope.int(hp.quniform("mlp_epochs", 10, 50, q=10)),
    },
    "cnn": {
        "num_blocks": scope.int(hp.quniform("cnn_blocks", 2, 5, 1)),
        "base_channels": scope.int(hp.quniform("cnn_base_channels", 16, 160, 16)),
        "channel_growth": hp.uniform("cnn_channel_growth", 1.2, 2.5),
        "use_batchnorm": hp.choice("cnn_use_bn", [True, False]),
        "dropout": hp.quniform("cnn_dropout", 0.1, 0.5, q=0.1),
        "lr": hp.loguniform("cnn_lr", -9, -3),
        "epochs": scope.int(hp.quniform("cnn_epochs", 10, 50, q=10)),
    },
    "resnet": {
        "layer1": scope.int(hp.quniform("resnet_layer1", 1, 4, 1)),
        "layer2": scope.int(hp.quniform("resnet_layer2", 1, 4, 1)),
        "layer3": scope.int(hp.quniform("resnet_layer3", 1, 4, 1)),
        "base_channels": scope.int(hp.quniform("resnet_base_channels", 16, 160, 16)),
        "lr": hp.loguniform("resnet_lr", -9, -3),
        "epochs": scope.int(hp.quniform("resnet_epochs", 10, 50, q=10)),
    },
    "convnext": {
        "depth1": scope.int(hp.quniform("convnext_depth1", 1, 4, 1)),
        "depth2": scope.int(hp.quniform("convnext_depth2", 1, 4, 1)),
        "depth3": scope.int(hp.quniform("convnext_depth3", 1, 4, 1)),
        "base_dim": scope.int(hp.quniform("convnext_base_dim", 48, 160, 16)),
        "dim_growth": hp.uniform("convnext_dim_growth", 1.2, 2.5),
        "lr": hp.loguniform("convnext_lr", -9, -3),
        "epochs": scope.int(hp.quniform("convnext_epochs", 10, 50, q=10)),
    },
}
